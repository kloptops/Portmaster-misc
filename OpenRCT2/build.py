...
valid = False
with zipfile.ZipFile('OpenRCT2.zip', 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9) as zf:
    zf.write('OpenRCT2.sh')

    for src, dest in build_files(Path('openrct2'), 'openrct2'):
        print(src, dest)
        zf.write(str(src), str(dest))

    for src, dest in build_files(Path('build/'), 'openrct2'):
        print(src, dest)
        zf.write(str(src), str(dest))
